# ---------------------------------------------------------------------------------------------#
#                                                                                              #
#                                 Mixture Based Model on K-Means                               #
#                                                                                              #                                                                                         #                                                                                             #
# ---------------------------------------------------------------------------------------------#
...
def estep(X: np.ndarray, mixture: GaussianMixture) -> np.ndarray:
    ...
    K, _ = mixture.mu.shape     # Numer of mixture components
    post = np.zeros((n, K))     # Initializing shape of posterior

    for i in range(n):
        tiled_vector = np.tile(X[i, :], (K, 1))             # Constructs an array by repeating the first elements K times in 1 dimension
        sse = ((tiled_vector - mixture.mu)**2).sum(axis=1)  # Sum of squared estimate of errors
        j = np.argmin(sse)                                  # Taking minimum
        post[i, j] = 1                                      # Posterior
...
def mstep(X: np.ndarray, post: np.ndarray) -> Tuple[GaussianMixture, float]:
    ...
    _, K = post.shape                                     # Number of mixture components

    n_hat = post.sum(axis=0)                              # Sum over posteriors
    p = n_hat / n                                         # Update weights
    ...
    for j in range(K):
        mu[j, :] = post[:, j] @ X / n_hat[j]              # Update means
        sse = ((mu[j] - X)**2).sum(axis=1) @ post[:, j]   # Update errors
        cost += sse                                       # Sum up errors
        var[j] = sse / (d * n_hat[j])                     # Update variances
